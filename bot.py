...
app = Client("renamer_bot", api_id=API_ID, api_hash=API_HASH, bot_token=BOT_TOKEN, workers=20)
tmdb = TMDBClient()
...
@app.on_message(filters.private & filters.user(ADMIN_IDS) & (filters.document | filters.video | filters.audio))
async def admin_file_to_link(client, message):
    file_name, file_id = None, None
    if message.video: file_name, file_id = message.video.file_name, message.video.file_id
    elif message.document: file_name, file_id = message.document.file_name, message.document.file_id
    elif message.audio: file_name, file_id = message.audio.file_name, message.audio.file_id

    file_size = message.video.file_size if message.video else (message.document.file_size if message.document else message.audio.file_size)
    mime_type = message.video.mime_type if message.video else (message.document.mime_type if message.document else message.audio.mime_type)
    ...
    if not host_url:
        return await message.reply_text("❌ `RENDER_EXTERNAL_URL` not set.")
    
    dl_link = f"{host_url.rstrip('/')}/dl/{short_id}"
    view_link = f"{host_url.rstrip('/')}/view/{short_id}"
    ...
    markup = InlineKeyboardMarkup([
        [InlineKeyboardButton("📥 Download File", url=dl_link)],
        [InlineKeyboardButton("🎬 Watch Online", url=view_link)]
    ])
...
async def main():
    ...
    web_app = await start_web_server(app)
